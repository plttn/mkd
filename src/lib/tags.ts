...
function parseRawTags(raw: unknown): string[] {
  if (!raw) return [];
  if (Array.isArray(raw)) return raw.map(String).flatMap(splitOnSeparators).map((s) => s.trim()).filter(Boolean);
  if (typeof raw === "string") return splitOnSeparators(raw).map((s) => s.trim()).filter(Boolean);
  return [];
}
